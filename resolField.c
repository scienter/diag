...
void main(int argc, char *argv[])
{
   ...
   case 0 :
     ...
     for(step=initial; step<=final; step+=timeStep)
     {
       ...
       restoreFloatArray(fileName,"X",dataX,nx);
       if(dimension>1) restoreFloatArray(fileName,"Y",dataY,ny); else ;
       if(dimension>2) restoreFloatArray(fileName,"Z",dataZ,nz); else ;

       restoreFieldComp(Ex,fileName,dataName,nx,ny,nz);

       if(dimension==1)	{ j=k=0; } else ;
       if(dimension==2)	{ j=ny/2; k=0; } else ;	
       if(dimension==3)	{ j=ny/2; k=nz/2;	} else ;

       if(dimension==1) {
         sprintf(fileName,"%s%d_%s_X",argv[8],step,argv[9]);
         out=fopen(fileName,"w");
         for(i=0; i<nx; i+=stX)
           fprintf(out,"%g %g\n",dataX[i],Ex[i][0][0]);
         fclose(out);
