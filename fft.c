...
int main(int argc, char *argv[])
{
   ...
   double tmp, dx, xrange, initialX,minX;
...
//   binOrder = atoi(argv[2]);
   ...
   {
     sprintf(fileName,"0Power%d",step);
//     sprintf(fileName,"0Field%d",step);
     in = fopen(fileName,"r");
     cnt=0; 
//   while(fscanf(in,"%lf %lf %lf %lf %lf %lf %lf",&tmp,&tmp,&tmp,&tmp,&tmp,&tmp,&tmp)!=EOF) 
...
//   maxX=dt*step*c-rU*4*lambda;
     ...
     for (j=0; j<cnt; j++)
     {
//      fscanf(in,"%lf %lf %lf %lf %lf %lf %lf",&xold[j],&oldData1[j],&oldData2[j],&oldData3[j],&oldData4[j],&oldData5[j],&oldData6[j]);
...
//      norData5=sqrt(Data5[i*2]*Data5[i*2]+Data5[i*2+1]*Data5[i*2+1]);
//      norData6=sqrt(Data6[i*2]*Data6[i*2]+Data6[i*2+1]*Data6[i*2+1]);
      f=2*M_PI*(i*1.0)/range;
//      f=lowFre/range;
//      lowFre=lowFre+1.0;
         fprintf(out,"%lf %lf %.10lf\n",f,norData1,norData2);
//       printf("%lf %lf %lf %lf %lf %lf %lf\n",f,norData1,norData2,norData3,norData4,norData5,norData6);
    }
    fclose(out);
    printf("%s is made\n",outFile);
