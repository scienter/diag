...
void saveParticleFile(double *data,double minPx,double minX,int cntSub,int step,int species,double minE,double maxE,int atomNum,double *trackID,int *trackCore,int cnt);
...
{
   int mode,species,initial,final,timeStep,atomNum,core;
   int i,n,step,cnt,totalCnt,cntSub,start,column,sharePNum,index;
   int *recv,numX,numY,*trackCore;
   double minPx,minE,maxE,minX,maxX,minY,maxY,minZ,maxZ;
   double slope,rangeX,tmp,id;
   double *data,*trackID;
   FILE *out,*in;
   ...
   if(mode==1) {
     ...
     if(myrank==0) {
       ...
       in=fopen(fileName,"r");
       if(in==NULL) { printf("find 'track' file\n"); exit(0); } else ;
       while(fscanf(in,"%lf %lf %lf %lf %lf %lf %lf %d %lf",&tmp,&tmp,&tmp,&tmp,&tmp,&tmp,&id,&core,&tmp)!=EOF)
         cnt++;
       fclose(in);

       trackID=(double *)malloc(cnt*sizeof(double ) );
       trackCore=(int *)malloc(cnt*sizeof(int ) );

       in=fopen(fileName,"r");
       for(i=0; i<cnt; i++)
         fscanf(in,"%lf %lf %lf %lf %lf %lf %lf %d %lf",&tmp,&tmp,&tmp,&tmp,&tmp,&tmp,&trackID[i],&trackCore[i],&tmp);
       fclose(in);
     } else ;
...
//     MPI_Bcast(&totalCnt,nTasks,MPI_INT,0,MPI_COMM_WORLD);
     ...
     MPI_Barrier(MPI_COMM_WORLD);
     MPI_Bcast(trackID,cnt,MPI_DOUBLE,0,MPI_COMM_WORLD);
     MPI_Bcast(trackCore,cnt,MPI_INT,0,MPI_COMM_WORLD);

if(myrank==0) {
       for(i=0; i<cnt; i++)
         printf("cnt=%d,id=%lf,core=%d\n",cnt,trackID[i],trackCore[i]);
}
   } else ;
...
//       MPI_Bcast(&totalCnt,nTasks,MPI_INT,0,MPI_COMM_WORLD);
       ...
       if(mode==1 || mode==2 || mode==4) {
         saveParticleFile(data,minPx,minX,cntSub,step,species,minE,maxE,atomNum,trackID,trackCore,cnt); 
       } else if(mode==3) {
...
void saveParticleFile(double *data,double minPx,double minX,int cntSub,int step,int species,double minE,double maxE,int atomNum,double *trackID,int *trackCore,int cnt)
{
   int i,n,nn,index,sharePNum,dataNum=9,intId;
   int *recvDataCnt;
   ...
   for(n=0; n<cntSub; n++)  {
     if(cnt==0) {
       if(data[n*dataNum+3]>minPx) sharePNum+=1; else ;
     }
     else {
       px=data[n*dataNum+3];
       id=data[n*dataNum+6];
       core=data[n*dataNum+7];
//printf("minPx=%g,n=%d,px=%g,id=%15g,core=%d\n",minPx,n,px,id,core);
       for(nn=0; nn<cnt; nn++)  {
         if(px>minPx && id==trackID[nn] && core==trackCore[nn]) {
//printf("myrank=%d, px=%g, id=%g, core=%d\n",myrank,px,id,core);
           sharePNum+=1;
           nn=cnt;
         } else;
       }
...
if(myrank==0)  {
  for(i=0; i<nTasks; i++)
   ...
   if(myrank==0)  {
     if(cnt==0) sprintf(fileName,"%dParticle%d",species,step);
     else       sprintf(fileName,"%did%d",species,step);
     out=fopen(fileName,"w");
     for(n=0; n<cntSub; n++)  {
       if(cnt==0) {
         if(data[n*dataNum+3]>minPx) {
