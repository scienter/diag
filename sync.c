...
#define hbarJ 1.054571817e-34		// [J .s]
...
void main(int argc, char *argv[])
{
   ...
   if(argc < 11)   { 
		if(myrank==0) {
	      printf("sync mode correction scrAngle scrD scrN minPhotonE maxPhotonE numPhoton screenX0 numeric jumpT\n");
   	   printf("mode 0 : energy B0 Ex undL lambdaU dz maxStep\n");
      	printf("mode 1 (PIC) : testMode(on:1 / off:0) jumpP\n");
   ...
   D->scrN=atoi(argv[5]);
	...
	for(i=0; i<D->numW; i++)  
	 ...
	 case 0 :
		...
		E0=atof(argv[14]);
		D->undL=atof(argv[15]);
		D->lambdaU=atof(argv[16]);
		dz=atof(argv[17]);
		D->maxStep=atoi(argv[18]);
	...
	case 2 :
		if(D->mode==2) {
			pickId=atof(argv[12]);
			pickCore=atof(argv[13]);
		} else if(D->mode==1)  {
			D->testMode=atoi(argv[12]);
			D->jumpP=atoi(argv[13]);
		} else ;
		...
		if(D->mode==1) {
			...
			for(i=start; i<end; i+=D->jumpP) 	{
				...
				if(pickId==id && pickCore==core) {
					restoreData(fileName,dataName,trackParticle);
					printf("myrank=%d, index=%d,id=%g, core=%g, weight=%g\n",myrank,i,id,core,weight);
					calRadiation(D,spectra1,spectra2,trackParticle,weight*D->jumpP,i,end);
				}	else ;
...
void calRadiation(Domain D,double ***spectra1,double ***spectra2,double *trackParticle,double weight,int index,int end)
{
	int i,ii,numW,idx,startI,maxStep,nn,jump,jumpI;
	...
	for(idx=0; idx<D->scrN; idx++)  
	{
		...
		for(ii=startI; ii<numW; ii++) 
		{
			...
			for(nn=jump; nn<maxStep-jump; nn+=jump) 
			{
				...
				if(jump%2==0) {
					for(i=0; i<3; i++) {
						nextU[i]=0.5*(trackParticle[(nn+jumpI)*6+i+3]+trackParticle[(nn+jumpI+1)*6+i+3]);
						prevU[i]=0.5*(trackParticle[(nn-jumpI)*6+i+3]+trackParticle[(nn-jumpI+1)*6+i+3]);
					}
				} else {
					for(i=0; i<3; i++)  {
						nextU[i]=trackParticle[(nn+1+jumpI)*6+i+3];	
						prevU[i]=trackParticle[(nn-jumpI)*6+i+3];
					}
				}
				if(nextU[1]!=0) {
					for(i=0; i<3; i++)  u[i]=0.5*(trackParticle[(nn+0)*6+i+3]+trackParticle[(nn+1)*6+i+3]);
					...
					for(i=0; i<3; i++) {
						u1[i]=(trackParticle[(nn+1)*6+i+3]-trackParticle[(nn+0)*6+i+3])/(1.0*D->dt);
						r1[i]=u[i]*c*invGamma;
...
{
	int i,ii,numW,idx;
	double minW,maxW,dW,w,sp1,sp2,coef,coef2,r,angle,sum[3],I0[3],dr,steradian,X0;
	double unitN;
	FILE *out1,*out2;
